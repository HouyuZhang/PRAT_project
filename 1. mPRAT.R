...
if(file.exists(paste0(mPRAT_prefix,"Seurat.rds"))){
  ...
  if(F){
    mPRAT_merged <- subset(mPRAT_merged, idents = c("ASPC", "Adipocyte", "Macrophage","Endothelial Cell", "Pericyte", "Mesothelial Cell"))
    mPRAT_merged <- ScaleData(mPRAT_merged) %>% RunPCA(npcs = 30) %>% RunUMAP(reduction = "pca", dims = 1:30) %>% FindNeighbors(reduction = "pca", dims = 1:30) %>%
      FindClusters(resolution = seq(from=0, by=0.05, length=5))
    clustree(mPRAT_ASPC_merged)
    ggsave(paste0(mPRAT_ASPC_prefix, "clustree_",Sys.Date(),".pdf"), width = 12, height = 10)
    saveRDS(mPRAT_merged, file = paste0(mPRAT_prefix,"Seurat.rds"))
  }
...
showCategoryNum <- c("response to cold","lipid catabolic process","carbohydrate catabolic process","fatty acid oxidation","ATP generation from ADP",
                     "positive regulation of lipid biosynthetic process","regulation of oxidoreductase activity","glucose transmembrane transport",
                     "triglyceride biosynthetic process","regulation of monooxygenase activity","fatty acid synthase activity",
                     "acetyl-CoA biosynthetic process","cellular lipid biosynthetic process")
