...
#===============================================================================
# 3. mPRAT-iWAT RT ASPC
#===============================================================================
mPRATiWATRT_ASPC_prefix <- "mPRAT-iWAT_RT_ASPC_"
if(file.exists(paste0(mPRATiWATRT_ASPC_prefix,"Seurat.rds"))){
  mPRATiWATRT_mPRATiWATRT_ASPC_merged <- readRDS(paste0(mPRATiWATRT_ASPC_prefix,"Seurat.rds"))
} else {
  mPRATiWATRT_ASPC_merged <- ScaleData(subset(mPRATiWATRT_merged, idents = c("ASPC"))) %>% RunPCA(npcs = 30) %>% RunUMAP(reduction = "pca", dims = 1:30) %>% 
    FindNeighbors(reduction = "pca", dims = 1:30) %>% FindClusters(resolution = seq(from=0, by=0.05, length=5))
  clustree(mPRATiWATRT_ASPC_merged)
  ggsave(paste0(mPRATiWATRT_ASPC_prefix, "clustree_",Sys.Date(),".pdf"), width = 12, height = 10)

  Idents(mPRATiWATRT_ASPC_merged) <- mPRATiWATRT_ASPC_merged$integrated_snn_res.0.05
  DimPlot(mPRATiWATRT_ASPC_merged, reduction = "umap", split.by = "orig.ident", ncol = 2, label = F, repel = T, label.box =T, order = T, cols = hughie_color)
  Plot_Cell_compoistion(Seurat_Obj = mPRATiWATRT_ASPC_merged, ColorUse1 = hughie_color, ColorUse2 = hughie_color, OutPrefix = mPRATiWATRT_ASPC_prefix)
  TopMarkersGenes <- MarkersPlot(Seurat_Obj = mPRATiWATRT_ASPC_merged, ColorUse = hughie_color, OutPrefix = mPRATiWATRT_ASPC_prefix)
}

mPRATiWATRT_ASPC_merged <- RenameIdents(mPRATiWATRT_ASPC_merged, `0` = "", `1` = "", `2` = "FAPs",`3` = "Preadipocytes")
mPRATiWATRT_ASPC_merged$Myannotation_ASPC <- mPRATiWATRT_ASPC_merged@active.ident
mPRATiWATRT_ASPCRanks <- c("Ad-1","Ad-2","Ad-3","Ad-4")
mPRATiWATRT_ASPC_colors <- c("#BC3C29FF","#E18727FF","#0072B5FF","#20854EFF")
names(mPRATiWATRT_ASPC_colors) <- mPRATiWATRT_ASPCRanks
Idents(mPRATiWATRT_ASPC_merged) <- factor(Idents(mPRATiWATRT_ASPC_merged), levels = mPRATiWATRT_ASPCRanks)

PlotGenes <- c("Pdgfra","Robo2","Hmcn1","Nos1",
               "Pdgfrb","Pparg","Acsl1","Adipor2",# Preadipocytes
               "Dpp4","Pi16","Cd55","Anxa3", #FAPs
               "Dock2","Ptprc","Pax5","Cr2","Ighd",# B cell like
               "Skap1","Lef1", #T cell like
               "Adgre1","Mctp1","Mrc1","Lgals3","Ccl6" #Macrophage like (Areg)
)

(p1 <- VlnPlot(mPRATiWATRT_ASPC_merged, features = PlotGenes, fill.by = "ident", cols = mPRATiWATRT_ASPC_colors, stack = T, flip=T, assay = "RNA") + FontSize(x.text = 16, y.text = 16) + NoLegend())
(p2 <- VlnPlot(mPRATiWATRT_ASPC_merged, features = PlotGenes, stack=T, flip=T, split.by = "orig.ident", cols = mPRATiWATRT_SAMcolors, assay = "RNA") + FontSize(x.text = 16, y.text = 16))
p1 + p2
ggsave(paste0(mPRATiWATRT_ASPC_prefix, "Selected_markers_VlnPlot_",Sys.Date(),".pdf"), width = 16, height = 12)

(p1 <- DimPlot(mPRATiWATRT_ASPC_merged, reduction = "umap", group.by = c("ident"), label = T, repel = T, label.box =T, cols = mPRATiWATRT_ASPC_colors, label.color = "white"))
(p2 <- DimPlot(mPRATiWATRT_ASPC_merged, reduction = "umap", group.by = c("orig.ident"), label.box =T, cols = mPRATiWATRT_SAMcolors))
p1 + p2
ggsave(paste0(mPRATiWATRT_ASPC_prefix, "UMAP-ByClusters_", Sys.Date(),".pdf"), width = 16, height = 8)

DimPlot(mPRATiWATRT_ASPC_merged, reduction = "umap", split.by = "orig.ident", ncol = 2, label = F, repel = T, label.box =T, cols = mPRATiWATRT_ASPC_colors, label.color = "white", order = T)
ggsave(paste0(mPRATiWATRT_ASPC_prefix,"UMAP-Byorig.ident_",Sys.Date(),".pdf"), width = 12, height = 10)

Plot_Cell_compoistion(Seurat_Obj = mPRATiWATRT_ASPC_merged, OutPrefix = mPRATiWATRT_ASPC_prefix, ColorUse1 = mPRATiWATRT_ASPC_colors, ColorUse2 = mPRATiWATRT_SAMcolors)
TopMarkersGenes <- MarkersPlot(Seurat_Obj = mPRATiWATRT_ASPC_merged, ColorUse = mPRATiWATRT_ASPC_colors, OutPrefix = mPRATiWATRT_ASPC_prefix, NMarkers = 20)
DEG_enrichment(Seruat_DEG_file = paste0(mPRATiWATRT_ASPC_prefix, "Allmarkers_",Sys.Date(),".csv"), showCategoryNum = 15, filterLevel = 4,cellRanks = mPRATiWATRT_ASPCRanks)

FeaturePlot(mPRATiWATRT_ASPC_merged, features =  c("Ucp1","Cidea","Esrrg","Ar","Cyp2e1","Trhde"), min.cutoff = "q9", order = T, pt.size=0.3)
ggsave(paste0(mPRATiWATRT_ASPC_prefix,"FeaturePlot_",Sys.Date(),".pdf"), width = 8, height = 10)
saveRDS(mPRATiWATRT_ASPC_merged, file = paste0(mPRATiWATRT_ASPC_prefix,"Seurat.rds"))
